...
def score_batting(conn, team, fmt, match_id) -> tuple[float, list, list]:
    ...
    if not xi:
        return 5.0, [], ["XI not confirmed — confirm playing XI to see batting signal"]
...
def score_bowling(conn, team, fmt, match_id) -> tuple[float, list, list]:
    ...
    if not xi:
        return 5.0, [], ["XI not confirmed — confirm playing XI to see bowling signal"]
...
def score_form(conn, team, fmt, match_id) -> tuple[float, list]:
    ...
    if not xi:
        return 5.0, ["XI not confirmed — confirm playing XI to see form signal"]
